...
def cantidad_arboles(df: dataframe) -> None:
    ...
    # Se muestran los primeros y ultimos nombres para verificar que se ordenaron
...
def mejor_ajuste(df: dataframe, mis_arboles: list):
    """
    Asignacion: 3
    ...
    for x in mis_arboles:
        # nuevo_df: dataframe auxiliar con los datos de un solo tipo de arbol
        nuevo_df = df[df['Nombre común'] == x]
        data = nuevo_df['Diámetro (cm)'].tolist()
        print(data)
        f = Fitter(data)
        f.fit()
        f.summary()
...
print('Mis dos árboles son: {} y {}.'.format(arboles_asign3[0], arboles_asign3[1]))
